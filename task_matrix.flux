...
filterData = (tables=<-) =>
  tables
    |> range(start: 2019-01-01T00:00:00Z, stop: 2019-01-11T23:59:59Z)
...
datafrom2019 = from(bucket:"veronacard_test")
|> filterData()

datafrom2019_again = from(bucket:"veronacard_test")
|> filterData()
|> map(fn: (r) => ({r with
        enum: r.enum-1
    })
...
|> set(key: "_measurement", value: "means")
|> to(
    // org: "example-org",
